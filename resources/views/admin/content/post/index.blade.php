                ...
                <section class="table-responsive">
                    <table class="table table-striped table-hover">
                        ...
                        <tr>
                            ...
                            <td>{{$post->slug}}</td>
                            <td><img src="{{asset($post->image['indexArray'][$post->image['currentImage']])}}" class="max-height-4rem"></td>
                            <td>
                                <label>
                                    <input type="checkbox" id="change_status_{{$post->id}}"
                                           onchange="changeStatus({{$post->id}})"
                                           data-url="{{route('admin.content.post.ajax.change-status',[$post->id])}}"
                                           @if($post->status==1) checked @endif>
                                </label>
                            </td>
                            <td>
                                <label>
                                    <input type="checkbox" id="change_commentable_{{$post->id}}"
                                           onchange="changeCommentable({{$post->id}})"
                                           data-url="{{route('admin.content.post.ajax.change-commentable',[$post->id])}}"
...
@section('scripts')
    <script type="text/javascript">
        ...
        function changeCommentable(id) {
            var element = $('#change_commentable_' + id);
            var url = element.attr('data-url');
            var elementValue=!element.prop('checked');
            ...
            $.ajax({
                ...
                success:function (response){
                    if (response.status){
                        if (response.checked){
                            element.prop('checked',true);
                            successToast('درج کامنت با موفقیت فعال شد');
                        }else {
                            element.prop('checked',false);
                            successToast('درج کامنت با موفقیت غیر فعال شد');
                        }
                    }else {
                        element.prop('checked',elementValue);
                        errorToast('خطا در تغییر وضعیت');
