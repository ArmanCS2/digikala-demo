            ...
            <section class="main-body-container">
                ...
                <section class="d-flex justify-content-between align-items-center mt-4 mb-3 border-bottom pb-2">
                    <a href="{{ route('admin.market.product.guarantee.create',[$product->id]) }}"
                       class="btn btn-info btn-sm">افزودن گارانتی جدید</a>
                    ...
                    <table class="table table-striped table-hover">
                        ...
                        @foreach($guarantees as $key => $guarantee)
                            <tr>
                                ...
                                <td>
                                    <label>
                                        <input type="checkbox" id="change_status_{{$guarantee->id}}"
                                               onchange="changeStatus({{$guarantee->id}})"
                                               data-url="{{route('admin.market.product.guarantee.ajax.change-status',[$guarantee->id])}}"
                                               @if($guarantee->status==1) checked @endif>
                                    </label>
                                </td>
                                <td class="width-22-rem text-left">
                                    <a href="{{route('admin.market.product.guarantee.edit',[$guarantee->id])}}"
                                       class="btn btn-primary btn-sm"><i class="fa fa-edit"></i> ویرایش</a>
                                    <form class="d-inline"
                                          action="{{route('admin.market.product.guarantee.destroy',[$guarantee->id])}}"
                                          method="post">
                                        @csrf
                                        @method('delete')
