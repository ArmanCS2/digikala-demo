        ...
        <section class="col-12">
            <section class="main-body-container">
                ...
                <section class="d-flex justify-content-between align-items-center mt-4 mb-3 border-bottom pb-2">
                    <a href="{{ route('admin.market.category.attribute.index') }}" class="btn btn-info btn-sm">بازگشت</a>
                </section>

                <section>
                    <form action="{{route('admin.market.category.attribute.update',[$attribute->id])}}" method="post">
                        @csrf
                        @method('put')
                        <section class="row">
                                    ...
                                    <select name="category_id" id="" class="form-control form-control-sm">
                                        ...
                                        @foreach($categories as $category)
                                            <option value="{{$category->id}}"
                                                    @if(old('category_id',$attribute->category_id)==$category->id) selected @endif>{{$category->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                                @error('category_id')
